...
#include "slic3r/GUI/ProfileCache.hpp"
...
int GuideFrame::LoadProfileFamily(std::string vendor_name, boost::filesystem::path vendor_json, boost::filesystem::path vendor_subdir)
{
    ...
    for (int n = 0; n < pModels.size(); n++) {
        ...
        boost::filesystem::path cover_path = boost::filesystem::absolute(boost::filesystem::path(resources_dir()) / "/profiles/" / vendor_name / strCover).make_preferred();
        if (!boost::filesystem::exists(cover_path)) {
            cover_path = (boost::filesystem::absolute(boost::filesystem::path(resources_dir()) / "/web/image/printer/") / strCover).make_preferred();
        }
